...
# from config import CARD_REQUISITES
# from config import VIP_CHANNEL_LINK
# from config import BAN_FILE

# router = Router()

# # --- РАБОТА С ФАЙЛОМ БЛОКИРОВОК ---
# def get_banned_users() -> set:
#     """Получить список заблокированных ID из файла"""
...
#     """Удалить ID из файла блокировок"""
#     banned = get_banned_users()
...
# # --- МИДЛВАРЬ ДЛЯ ПРОВЕРКИ БАНА (ОХРАННИК) ---
# class BanCheckMiddleware(BaseMiddleware):
#     async def __call__(self, handler, event: TelegramObject, data: dict):
#         user = data.get("event_from_user")
        
#         # Если пользователь есть в черном списке
#         if user and user.id in get_banned_users():
#             if isinstance(event, Message):
#                 await event.answer("❌ <b>Доступ ограничен.</b>\nВы заблокированы администратором за нарушение правил (спам/флуд).")
#             elif isinstance(event, CallbackQuery):
#                 await event.answer("❌ Доступ ограничен. Вы заблокированы.", show_alert=True)
#             return  # СТОП: дальше код не идет, бот не ответит на команды
            
#         return await handler(event, data)

# # Состояния FSM
# class BotStates(StatesGroup):
#     waiting_for_question = State()  # Ожидание вопроса в техподдержку
#     waiting_for_receipt = State()   # Ожидание скриншота чека от юзера
#     # Состояния для админки
#     admin_waiting_ban = State()     # Админ вводит ID для бана
#     admin_waiting_unban = State()   # Админ вводит ID для разбана


# # Главная клавиатура для пользователей
# main_kb = InlineKeyboardMarkup(inline_keyboard=[
#     [InlineKeyboardButton(text="💎 Вип канал (Подписка)", callback_data="vip_channel")],
#     [InlineKeyboardButton(text="🆘 Помощь админа", callback_data="admin_help")]
# ])
...
# # Вход в админку по команде /admin (доступно только админу)
...
#     await callback.message.edit_reply_markup(reply_markup=None) # Убираем кнопки, чтобы не спамить
#     await callback.message.answer("Введите Telegram ID пользователя, которого нужно <b>заблокировать</b>:")
...
#     await message.answer(f"✅ Пользователь с ID <code>{target_id}</code> успешно <b>заблокирован</b>. Бот для него больше не работает.", parse_mode=ParseMode.HTML)
#     await state.clear()

# # Кнопка: Разблокировать по ID
...
#     await callback.message.answer("Введите Telegram ID пользователя, которого нужно <b>разблокировать</b>:")
#     await state.set_state(BotStates.admin_waiting_unban)
#     await callback.answer()

# # Ловим ID для разбана
...
#         f"Привет, {message.from_user.first_name}! Выбери нужное действие в меню ниже:",
#         reply_markup=main_kb
#     )


# # # 2. Кнопка "Вип канал" — выдаем реквизиты и включаем ожидание чека
# # @router.callback_query(F.data == "vip_channel")
# # async def send_requisites(callback: CallbackQuery, state: FSMContext):
# #     await callback.message.answer(
# #         f"{CARD_REQUISITES}\n\n"
# #         "🧾 <b>После оплаты обязательно пришлите сюда скриншот или фото чека.</b>\n"
...
# # # 3. Ловим чек от пользователя (принимаем только фото)
# # @router.message(BotStates.waiting_for_receipt, F.photo)
# # async def handle_receipt(message: Message, state: FSMContext, bot: Bot):
# #     user_id = message.from_user.id
# #     user_name = message.from_user.username or message.from_user.first_name
    
# #     # Берем самое лучшее качество фото из массива
# #     photo_id = message.photo[-1].file_id
    
# #     # Клавиатура для админа (в callback_data зашиваем ID покупателя)
# #     admin_pay_kb = InlineKeyboardMarkup(inline_keyboard=[
# #         [
# #             InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"pay_approve:{user_id}"),
...
# #     await message.answer("⏳ Ваш чек успешно отправлен администратору. Ожидайте подтверждения и ссылку!")
# #     await state.clear()


# # # Если пользователь вместо фото чека отправил текст
# # @router.message(BotStates.waiting_for_receipt)
# # async def handle_receipt_wrong_format(message: Message):
# #     await message.answer("⚠️ Пожалуйста, отправьте чек именно <b>Фотографией</b> или <b>Скриншотом</b>.")

# # 2. Кнопка "Вип канал" — выдаем реквизиты и просим PDF
# @router.callback_query(F.data == "vip_channel")
# async def send_requisites(callback: CallbackQuery, state: FSMContext):
#     await callback.message.answer(
#         f"{CARD_REQUISITES}\n\n"
#         "🧾 <b>После оплаты обязательно отправьте сюда PDF-файл с чеком.</b>\n"
#         "(В приложении банка нажмите «Поделиться чеком» -> Telegram -> выберите этого бота).\n"
#         "Я сразу передам его администратору на проверку.",
#         parse_mode=ParseMode.HTML
#     )
#     await state.set_state(BotStates.waiting_for_receipt)
...
# # 3. Ловим чек от пользователя (принимаем только документы)
# @router.message(BotStates.waiting_for_receipt, F.document)
# async def handle_receipt(message: Message, state: FSMContext, bot: Bot):
#     # Проверяем, что отправленный документ — это именно PDF
#     if message.document.mime_type != 'application/pdf':
#         await message.answer("⚠️ Пожалуйста, отправьте чек именно в формате <b>PDF</b>.")
#         return
...
#     # Отправляем PDF ДОКУМЕНТ админу (используем send_document вместо send_photo)
#     await bot.send_document(
#         chat_id=ADMIN_ID,
#         document=document_id,
#         caption=f"💰 <b>Новый чек (PDF) на проверку!</b>\nОт: @{user_name} (ID: <code>{user_id}</code>)",
#         reply_markup=admin_pay_kb,
#         parse_mode=ParseMode.HTML
#     )
...
# # Если пользователь вместо PDF документа отправил текст или фото
# @router.message(BotStates.waiting_for_receipt)
# async def handle_receipt_wrong_format(message: Message):
#     await message.answer(
#         "⚠️ Пожалуйста, отправьте чек именно Файлом (PDF).\n"
#         "Если вы используете Kaspi или другой банк, нажмите в чеке кнопку «Поделиться» и выберите Telegram."
#     )

# # 4. Обработка решения админа (ПОДТВЕРДИТЬ)
...
#     # Создаем клавиатуру с кнопкой связи, которая будет висеть под ссылкой постоянно
#     success_kb = InlineKeyboardMarkup(inline_keyboard=[
#         [InlineKeyboardButton(text="🆘 Связь с админом", callback_data="admin_help")]
#     ])
    
#     try:
#         # Отправляем ссылку пользователю вместе с кнопкой поддержки
#         await bot.send_message(
#             chat_id=user_id,
#             text=(
#                 f"🎉 <b>Ваша оплата успешно подтверждена!</b>\n\n"
#                 f"Добро пожаловать в VIP-канал! Вот ваша персональная ссылка для входа:\n{VIP_CHANNEL_LINK}\n\n"
#                 f"<i>⚠️ Если ссылка станет недействительной или возникнут вопросы, "
#                 f"вы всегда можете нажать кнопку «Связь с админом» ниже.</i>"
#             ),
#             parse_mode=ParseMode.HTML,
#             disable_web_page_preview=True,
#             reply_markup=success_kb  # Добавляем нашу кнопку
#         )
        
#         # Обновляем сообщение у админа, убирая кнопки выбора
#         await callback.message.edit_caption(
#             caption=f"{callback.message.caption}\n\n🟢 <b>Оплата одобрена! Ссылка и кнопка поддержки отправлены.</b>",
#             parse_mode=ParseMode.HTML
#         )
#     except Exception as e:
#         await callback.message.answer(f"❌ Не удалось отправить ссылку пользователю. Ошибка: {e}")
#         logging.error(f"Ошибка апрува: {e}")
        
#     await callback.answer()

# # 5. Обработка решения админа (ОТКЛОНИТЬ)
...
#         await bot.send_message(
#             chat_id=user_id,
#             text="❌ <b>Ваша оплата была отклонена администратором.</b>\nЕсли это произошло по ошибке или у вас возникли вопросы, обратитесь в поддержку через главное меню.",
#             parse_mode=ParseMode.HTML
#         )
        
#         # Обновляем сообщение у админа
#         await callback.message.edit_caption(
#             caption=f"{callback.message.caption}\n\n🔴 <b>Оплата отклонена вами.</b>",
#             parse_mode=ParseMode.HTML
#         )
#     except Exception as e:
#         await callback.message.answer(f"❌ Не удалось отправить уведомление пользователю. Ошибка: {e}")
#         logging.error(f"Ошибка реджекта: {e}")
        
#     await callback.answer()


# # 6. Кнопка "Помощь админа"
# @router.callback_query(F.data == "admin_help")
# async def ask_admin_help(callback: CallbackQuery, state: FSMContext):
#     # Убираем клавиатуру (кнопки) у сообщения, на которое нажал пользователь
#     try:
#         await callback.message.edit_reply_markup(reply_markup=None)
#     except Exception:
#         pass # Игнорируем ошибку, если кнопки уже были удалены

#     await callback.message.answer(
#         "✍️ Напишите ваш вопрос одним сообщением, и я передам его администратору:\n\n"
#         "<i>(Если вы нажали случайно, отправьте /start для возврата в меню)</i>",
#         parse_mode=ParseMode.HTML
#     )
...
#         f"От: @{user_name} (ID: <code>{user_id}</code>)\n\n"
#         f"<b>Текст:</b> {message.text}\n\n"
#         f"<i>⚠️ Чтобы ответить, сделай Reply (Ответить) прямо на это сообщение.</i>"
#     )
...
# # 8. Обработка текстового ответа админа (через Reply)
...
#         logging.error(f"Ошибка при текстовом ответе: {e}")
...
#         # Если пользователь определен и его ID есть в списке банов
#         if user and user.id in get_banned_users():
            
#             # 1. ЖЕСТКИЙ СБРОС: Выкидываем его из любых ожиданий (вопроса/чека)
#             state: FSMContext = data.get("state")
#             if state:
#                 await state.clear()
                
#             # 2. Уведомляем о блокировке
#             if isinstance(event, Message):
#                 await event.answer("❌ <b>Доступ ограничен.</b>\nВы заблокированы администратором.")
#             elif isinstance(event, CallbackQuery):
#                 await event.answer("❌ Доступ ограничен. Вы заблокированы.", show_alert=True)
                
#             # 3. ПОЛНАЯ ОСТАНОВКА: код дальше не идет, админу ничего не пересылается
#             return 
...
#     # 1. Отдельное сообщение с прайс-листом и описанием
#     prices_text = (
...
#     # 2. Отдельное короткое сообщение с прикрепленными кнопками
#     await message.answer(
...
# # Ловим нажатие на любую из 3х подписок
# @router.callback_query(F.data.startswith("sub_"))
# async def send_requisites(callback: CallbackQuery, state: FSMContext):
#     # Вытаскиваем название (VIP, PREMIUM или ULTRA) из callback_data
#     sub_type = callback.data.split("_")[1] 
    
#     # Сохраняем выбранный тариф в память бота для этого юзера
#     await state.update_data(selected_sub=sub_type)
    
#     # Убираем кнопки меню, чтобы не нажимали дважды
#     try:
#         await callback.message.edit_reply_markup(reply_markup=None)
#     except Exception:
#         pass
...
#     # Достаем название тарифа из памяти
#     data = await state.get_data()
#     sub_type = data.get("selected_sub", "Неизвестно")
    
#     # Зашиваем тариф в кнопки для админа, чтобы при нажатии бот знал, какую ссылку дать
#     admin_pay_kb = InlineKeyboardMarkup(inline_keyboard=[
#         [
...
# async def handle_receipt_wrong_format(message: Message):
#     await message.answer("⚠️ Пожалуйста, отправьте чек именно в формате PDF.\n\n"
#                          "⚠️ Отыныш, чекты <b>PDF</b> форматта жберу")

# # Обработка подтверждения (теперь выдает РАЗНЫЕ ссылки)
# @router.callback_query(F.data.startswith("pay_approve:"))
# async def process_payment_approve(callback: CallbackQuery, bot: Bot):
#     # Разбиваем callback_data: ["pay_approve", "123456789", "VIP"]
#     parts = callback.data.split(":")
#     user_id = int(parts[1])
#     sub_type = parts[2]
    
#     # Достаем нужную ссылку из словаря
#     link = SUBSCRIPTION_LINKS.get(sub_type, "Ссылка не найдена")

#     success_kb = InlineKeyboardMarkup(inline_keyboard=[
#         [InlineKeyboardButton(text="🆘 Связь с админом\n\n"
#                               "🆘 Админнан комек", callback_data="admin_help")]
#     ])
#     try:
#         await bot.send_message(
...
#             chat_id=user_id,
#             text="❌ <b>Ваша оплата была отклонена администратором.</b>\n\n"
#             "❌ <b>Сыздын заявканыз кабылданбады.</b>",
#             parse_mode=ParseMode.HTML
#         )
#         await callback.message.edit_caption(
...
#                                   "✍️ Маселены быр созбен жазыныз, мен оны админга жберемын:")
#     await state.set_state(BotStates.waiting_for_question)
#     await callback.answer()
...
#         await bot.send_message(chat_id=ADMIN_ID, text=admin_text, parse_mode=ParseMode.HTML)
#         await message.answer("✅ Ваш вопрос успешно отправлен! Ожидайте ответа.\n\n"
#                              "✅ Сыздын заявканыз жберылды! Керы байланыс кутыныз.")
#     except Exception:
...
#                                "👨‍💻 <b>Админнан керы байланыс:</b>", parse_mode=ParseMode.HTML)
...
# BAN_FILE = os.getenv("BAN_FILE", "banned_users.txt")
# # Восстанавливаем переносы строк, записанные в .env как \n
# CARD_REQUISITES = os.getenv("CARD_REQUISITES", "").replace("\\n", "\n")

# # Собираем словарь ссылок
...
# # Подключаем защиту от заблокированных пользователей к нашему роутеру
